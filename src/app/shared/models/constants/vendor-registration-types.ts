...
export const FormsToShowForService: { [key in FormsList]: string[] } = {
  ...
  [FormsList.DriverDetails]: [Services.Driver]
}
export const apiEndPoints: { [key in Services]: IApiEndPoint } = {
  ...
  [Services.Driver]: { service: 'driver' }
}
...
export const DriverTabList: { [key: number]: string } = {
  0: FormsList.DriverDetails
};
...
export type RiderAndDriverStatus = 'draft' | 'pending' | 'approved' | 'rejected';
export const RiderAndDriverStatusList: {[key in RiderAndDriverStatus]: string} = {
  draft: 'Draft',
  pending: 'Pending',
  approved: 'Approved',
  rejected: 'Rejected',
...
export const OutletCardStatusList: {[key in OutletStatus | RiderAndDriverStatus]: string} = {
 ...OutletStatusList,
 ...RiderAndDriverStatusList
}
...
export const allowRouteAccessTo: { [key: string]: Roles[] } = {
  ...
  '/home/driver': [Roles.Admin],
  '/home/driver-history': [Roles.Admin],
}
...
export class RiderAndDriverDetails {
  approvalStatus: RiderAndDriverStatus;
  name: string;
  type: string;
  contact: string;
  dob: string;
  ...
  static fromJson(data: any): RiderAndDriverDetails {
    const r: RiderAndDriverDetails = new RiderAndDriverDetails();

    r['approvalStatus'] = data['approval_status'];
    r['name'] = data['name'];
    r['type'] = data['type'];
    r['contact'] = data['phone'];
    r['dob'] = moment(data['date_of_birth'], 'DD-MM-YYYY').format('YYYY-MM-DD'),
    r['photoDoc'] = data['image']?.['name'];
    r['photoUrl'] = data['image']?.['url'];
...
export class OutletCard {
  id: string;
  name: string;
  status: OutletStatus | RiderAndDriverStatus;
  approvedBy: string;
  updatedAt: string;
  imgUrl: string;
  service: Services;
