...
import { tabList, Services, RiderTabList, FormsList, FormsToShowForService, Roles, VendorDetails, RiderAndDriverDetails, OutletStatus, RiderAndDriverStatus, DriverTabList } from '../../../shared/models/constants/vendor-registration-types'
...
export class CreateVendorComponent implements OnInit, AfterViewInit {
  ...
  currentRiderStatus: RiderAndDriverStatus;
  riderStatusWithEditAccess: RiderAndDriverStatus[] = ['draft', 'rejected'];
  ...
  riderDetails: RiderAndDriverDetails;
  ...
  ngOnInit(): void {
    ...
    if (this.service === Services.Driver) {
      this.currentTab = DriverTabList;
      this.navBarList = Object.values(DriverTabList)
    }
   ...
   */
  setRiderDetails() {
    this.vendorService.getRiderAndDriverDetails(this.outletId).subscribe(res => {
      this.riderDetails = RiderAndDriverDetails.fromJson(res['result']);
      const path = this.activeRoute.snapshot.url[0].path;
